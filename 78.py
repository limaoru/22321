...
from store_config import CAMERA_CONFIGS, get_cam_lines, get_cam_zones, load_zone_config
...
# 区域/越线从 zone_config.json 加载（用 zone_editor.py 鼠标标定）
load_zone_config()
# =========================================================

# OpenCV 不支持中文，用 Pillow 批量渲染避免满屏 ???
...
def queue_text(img: np.ndarray, text: str, x: int, y: int, color=(255, 255, 255), size: int = 20):
    """收集文字，稍后 flush_text 一次性绘制（中文走 Pillow）"""
    _TEXT_BATCH.setdefault(id(img), []).append((text, x, y, color, size))


def flush_text(img: np.ndarray):
    """将 queue_text 收集的文字绘制到图像上"""
...
class StoreCamAnalytics:
    ...
    def draw_zones(self, frame):
        ...
        for zone in self.zones:
            ...
            queue_text(frame, f"{zone['name']}:{self.zone_current.get(zone['name'], 0)}",
                       cx - 40, cy, zone["color"], 18)
...
def export_live_state(cam_analytics, customer_counts, fps_state, metrics_by_cam):
    """导出实时状态供 Web 仪表盘读取"""
    if not ENABLE_WEB_DASHBOARD:
        return
    payload = {"updated": datetime.now().isoformat(timespec="seconds"), "cameras": {}}
    for cam_name, analytics in cam_analytics.items():
        m = metrics_by_cam.get(cam_name, {})
        payload["cameras"][cam_name] = {
...
def start_advanced_store_analytics():
    ...
    print("📐 区域标定: python3 store/zone_editor.py")
    print("📊 Web仪表盘: python3 store/dashboard.py  ->  http://127.0.0.1:5050")
    ...
    with torch.inference_mode():
        while True:
            ...
            if ENABLE_WEB_DASHBOARD and now - last_web_export >= WEB_EXPORT_INTERVAL:
                export_live_state(cam_analytics, customer_counts, fps_state, metrics_by_cam)
                last_web_export = now
