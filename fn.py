...
class RTSPStreamReader(threading.Thread):
    ...
    def run(self):
        print(f"📡 [线程启动] 正在拉起 {self.name} 的 FFmpeg 管道...")
        process = subprocess.Popen(
            shlex.split(self.ffmpeg_cmd), stdout=subprocess.PIPE, stderr=subprocess.DEVNULL, bufsize=self.frame_size
        )
...
def start_advanced_store_analytics():
    ...
    print("🔥 5060 多维度零售视界系统启动！加载中型算法 YOLOv8m...")
